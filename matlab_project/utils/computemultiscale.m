function [uv, latentImg, ppIMF, invppIMF] ...
    = computemultiscale(src, ref, idxSrc, idxRef, ROI)
...
% Init NNF 
% WITH SHIFTED INDICES, -1 IF GHOST AREA

newSize = [size(pyramidSrc{pyramid_levels}, 1) size(pyramidSrc{pyramid_levels}, 2)];   
scaledROI = zeros(newSize);
sc = 2^(pyramid_levels-1);
    
fprintf("\nIn 1 computemultiscale: scaledROI size = %d, %d", size(scaledROI, 1), size(scaledROI, 2));
    
for x = 1:size(scaledROI, 2)
    for y = 1:size(scaledROI, 1)
        scaledROI(y, x) = ROI(max(1,floor(y/sc)), max(1,floor(x/sc)));
    end
end
    
patchSize = 10;
[uv] = computehisteqnnf(pyramidRef{pyramid_levels}, pyramidSrc{pyramid_levels}, ROI);  % calls nnmex
...
uv(:,:,2) = padarray(uv(1:end-patchSize,1:end-patchSize,2), ...
    [patchSize, patchSize], 'replicate', 'post'); 
% Init IMF and inverse IMF - INTENSITY MAPPING FIELD?
...
for iLevel = pyramid_levels:-1:1
    ...
    %MY MAGIC:
    maskNotClippIMFedPixels = ones(size(maskNotClippIMFedPixels));
    %MY MAGIC END
    ...
    fprintf("\nIn computemultiscale: scaledROI size = %d, %d", size(scaledROI, 1), size(scaledROI, 2));
    ...
    [uv, latentImg, ppIMF, invppIMF] = ...\
        computesinglescale(pyramidSrc{iLevel}, pyramidRef{iLevel}, idxSrc, idxRef, min(30,maxIters), ...\
        uv, intensityMappedRef, intensityMappedSrc, latentImg, ppIMF,invppIMF, ...\
        dist_weight, maskClippIMFedPixels, patchSize, uvPrior, scaledROI);
