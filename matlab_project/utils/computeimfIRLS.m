...
[ppNew, ~,~, exitflag] = lsqlin(sparse(wXX), wYY, C, d, [], [], [], [], wXX\wYY, options);

if exitflag == -2
    ppNew = lsqlin(sparse(wXX),wYY,C,d,[],[],[],[],[],options);    
elseif exitflag == 0
    while exitflag == 0                
       ppOld = ppNew;
       [ppNew, ~,~, exitflag] = lsqlin(sparse(wXX), wYY, C, d, [], [], [], [], ppOld, options); 
...
%    fprintf('%d ', convergence)
    ...
    elseif exitflag == 0
        while exitflag == 0 && count < 1000
            count = count + 1;
            fprintf('count = %d ', count)
            [ppNew, ~,~, exitflag] = lsqlin(sparse(wXX), wYY, C, d, [], [], [], [], ppNew, options); 
        end
    end
