...
void mexFunction(int nout, mxArray *pout[], int nin, const mxArray *pin[]) {
  ...
  if (nin < 3) { mexErrMsgTxt("nnmex called with < 3 input arguments"); }

  const mxArray *A = pin[0], *B = pin[1], *inROI=pin[2];
  ...
  if (mxGetNumberOfDimensions(A) != 3 || mxGetNumberOfDimensions(B) != 3) { mexErrMsgTxt("dims != 3 for A or B"); }
  ...
  cerr << "Converting ROI in nnmex.cpp ..." << endl;
  BOOLMAP *ROI = convert_boolmap(inROI);
          /*create_boolmap(900, 1350);
  for (int y = 0; y < ROI->h; y++) {
		bool *row = (bool *) ROI->line[y];
		for (int x = 0; x < ROI->w; x++) {
			row[x] = bool((mxGetInt8s(inROI))[y*ROI->w + x]);
		}
  }*/
  cerr << "ROI created successfully" << endl;
...
//      sort_knn(p, vann, vann_sim, vannd);
      ...
      ann_sim_final = ann_sim;
        ...
        cout << "nnmex: else case, calling nn" << endl;
      ann = init_nn(p, a, b, bmask, NULL, amaskm, 1, ann_window, awinsize, ROI);
      BITMAP *annd = init_dist(p, a, b, ann, bmask, NULL, amaskm, ROI);
      nn(p, a, b, ann, annd, amaskm, bmask, 0, 0, rp, 0, 0, 0, NULL, p->cores, ann_window, awinsize, ROI); 
      if (ann_prev)
      {
        //  cout << "if ann_prev = true" << endl;
          minnn(p, a, b, ann, annd, ann_prev, bmask, 0, 0, rp, NULL, amaskm, p->cores, ROI);  
      }
      annd_final = annd;
      cout << "nnmex: else case end" << endl;
    }
  } 
/*
  else if (mode == MODE_VECB) {
